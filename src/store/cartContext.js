  ...
  function addToCart(data, amount) {
    ...
    if (isInCart(data.id)) {
      ...
      if (data.stock > 0) {
        ...
        toast(`Has agregado ${amount}, de ${data.name} al carrito`);
      } else {
        toast(
          `No se puede agregar más de ${itemIndex.stock}, de ${itemIndex.name} ya que no hay más en stock`
        );
      }
    }
